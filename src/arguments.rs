...
pub fn parse_arguments() -> anyhow::Result<ParsedCliArgs> {
    ...
    // Lexically normalize library search paths
    options.library_paths = options
        .library_paths
        .into_iter()
        .map(lexically_normalize_path)
        .collect::<Vec<_>>();
...
fn lexically_normalize_path(path: impl AsRef<Path>) -> PathBuf {
